...
func Build(ctx context.Context, client *containerd.Client, options types.BuilderBuildOptions) error {
	...
	if options.CredentialConfigFilePath != "" {
		if strings.HasPrefix(options.CredentialConfigFilePath, "fd://") {
			...
			log.L.Debugf("In fd %d", fd)
			f := os.NewFile(uintptr(fd), "")
			buildctlArgs = append(buildctlArgs, "--credential-config-file", fmt.Sprintf("fd://%d", strconv.Itoa(3)))
			log.L.Debugf("Added path")
			buildctlCmd.ExtraFiles = []*os.File{f}
		} else {
			// path
