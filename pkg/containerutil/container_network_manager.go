...
func loadDNSConfig(dns *[]string, dnsSearch *[]string, dnsOptions *[]string) error {
	if len(*dns) == 0 || len(*dnsSearch) == 0 || len(*dnsOptions) == 0 {
		...
		if len(*dns) == 0 {
			*dns = resolvconf.GetNameservers(conf.Content, resolvconf.IPv4)
		}
		if len(*dnsSearch) == 0 {
...
func getDNSConfig(netOpts types.NetworkOptions, dns *[]string, dnsSearch *[]string, dnsOptions *[]string) {
...
func (m *noneNetworkManager) ContainerNetworkingOpts(_ context.Context, containerID string) ([]oci.SpecOpts, []containerd.NewContainerOpts, error) {
	...
	getDNSConfig(m.netOpts, &dns, &dnsSearch, &dnsOptions)
	err = loadDNSConfig(&dns, &dnsSearch, &dnsOptions)
	if err != nil {
		return nil, nil, err
	}
