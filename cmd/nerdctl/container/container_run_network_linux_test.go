			...
			},
			{
				...
				Expected: func(data test.Data, helpers test.Helpers) *test.Expected {
					return &test.Expected{
						Output: func(stdout string, info string, t *testing.T) {
							containerName1 := data.Get("containerName1")
							fmt.Println(helpers.Capture("exec", containerName1, "cat", "/etc/hosts"))
							assert.Assert(t, strings.Compare(strings.TrimSpace(helpers.Capture("exec", containerName1, "cat", "/etc/hosts")), strings.TrimSpace(stdout)) == 0, info)
						},
					}
				},
			},
			{
				...
				Expected: func(data test.Data, helpers test.Helpers) *test.Expected {
					return &test.Expected{
						Output: func(stdout string, info string, t *testing.T) {
							containerName1 := data.Get("containerName1")
							assert.Assert(t, strings.Compare(strings.TrimSpace(helpers.Capture("exec", containerName1, "cat", "/etc/resolv.conf")), strings.TrimSpace(stdout)) == 0, info)
						},
					}
				},
			},
		...
		SubTests: []*test.Case{
			{
				...
				Command: func(data test.Data, helpers test.Helpers) test.TestableCommand {
					containerName1 := data.Get("containerName1")
					cmd := helpers.Command()
					cmd.WithArgs("run", "--name", data.Identifier(),
